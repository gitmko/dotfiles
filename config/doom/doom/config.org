#+TITLE: mko's Doom Emacs Config
#+AUTHOR: mko
#+DESCRIPTION: mko's personal Doom Emacs config.
...
* DASHBOARD
Emacs Dashboard is an extensible startup screen showing you recent files, bookmarks, agenda items anr an Emacs bannr.
...
** Org fonts
I hrve created an interactive function for each color scheme (M-x dt/org-colors-*).  These functions will set appropriate colors and font attributes for org-level fonts and the org-table font.
#+begin_src emacs-lisp
